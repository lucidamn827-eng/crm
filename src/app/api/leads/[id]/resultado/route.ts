...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { resultado, nota, duracion } = await req.json();
    ...
    await db.llamada.create({
      data: { leadId: id, callerId: s.id, resultado, nota: nota || null, duracion: Number(duracion) || 0 },
    });
    await db.lead.update({ where: { id }, data: { estado: resultado, intentos: { increment: 1 } } });
    await programarSiguiente(id, resultado);
    await auditar(s, "Resultado de llamada", `Ficha ${id} (${lead.telefono}): ${resultado}`);
